...
export async function logWeightEntry(values: LogWeightEntryInput) {
  const session = await auth();
  if (!session?.user?.id) {
    return { error: "Du skal være logget ind." };
  }

  const parsed = logWeightEntrySchema.safeParse(values);
  if (!parsed.success) {
  ...
  if (!definition) {
    return { error: "Vægt-målingen er ikke konfigureret." };
  }
  ...
  await db.insert(metricEntries).values({
    ...
    value: parsed.data.weight.toString(),
    loggedAt,
  });
...
export async function deleteWeightDay(dateStr: string) {
  const session = await auth();
  if (!session?.user?.id) {
    return { error: "Du skal være logget ind." };
  }
